...
`params.initial_mode` will return the value of the `initial_mode` parameter passed to this instance, and `state.mode!` initializes (or updates) the `mode` state variable.  State variables are like *reactive* instance variables.  They can only be changed using the "!" method, and when they change they will cause a rerender.  Note there is no "!" method for params - they cannot be changed internally by the component. 
